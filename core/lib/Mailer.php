	...
	class Mailer
	{
		...
		function Send($template, $subject= '', $sender_email = '', $sender_name = '')
		{
			...
			$mail->IsSMTP();								// telling the class to use SMTP
  			$mail->Host			= $config['emil']['host'];			// SMTP server
  			$mail->Port			= $config['emil']['port']
  
  			$mail->SMTPAuth		= true;						// enable SMTP authentication
  			$mail->Username		= $config['emil']['username'];	// SMTP account username
  			$mail->Password		= $config['emil']['password'];				// SMTP account password
