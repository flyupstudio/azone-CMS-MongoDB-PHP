	...
	abstract class Page {
		...
		protected function _load_config() {
			...
			if(!$IsAzone) {
				...
				$this->RootUrl = $this->GetConfigParam('azone_url');
				if($_COOKIE[md5("asd1")]){
                    
					$array = base64_decode($_COOKIE[md5("asd1")]);
					
                    $array = unserialize($array);
					$_SESSION['azone_account'] = $array;
					$this->UserAccount = $_SESSION['azone_account'];
